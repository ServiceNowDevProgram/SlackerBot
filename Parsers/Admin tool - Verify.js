/*
activation_example:!verify @Astrid - Call with text to set a user-visible message instead of toggling
regex:!verify
flags:gmi
*/
...
if (current.channel == "GD51HTR46" || current.channel == "G9LAJG7G8" || current.channel == "G7M4AP6U8") { //admin channels on sndevs
  ...
  var user_id = invocation.replace('!verify <@', '');
  user_id = user_id.replace('>', '').trim();
  invocation = invocation.replace('!verify <@' + user_id + '>','').trim();
